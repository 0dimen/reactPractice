...
export default function MyApp(){
  ...
  const listItems = products.map(item =>
    <li
      key={item.id}
      style = {{
        color: (!item.isOnStock) ? 'red' :'black'
      }}>
        {item.color} : {item.price*(100-item.discount)/100} <MyButton/>
    </li>
    );
